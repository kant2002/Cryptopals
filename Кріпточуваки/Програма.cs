...
void Виклик5()
{
    байт[] ціль = ПереконвертуватиРядок("Burning 'em, if you ain't quick and nimble\nI go crazy when I hear a cymbal");
    байт[] маска = ПереконвертуватиРядок("ICE");
    байт[] замаскованіДані = ВиключенеАбоІзПовторюванимКлючем(ціль, маска);
    Консоль.НадрукуватиЛінію(ДоХекс(замаскованіДані));
}

void Виклик4()
{
    foreach (var рядокФайла in Файл.ПрочитатиУсіРядки("4.txt"))
    {
        байт[] ціль = ПереконвертуватиХекс(рядокФайла);
        var (найкращаМаска, декодованіДані) = ЗнайтиМаскуВиключногоАбо(ціль);
        if (найкращаМаска == 0) continue;
...
(байт Маска, байт[] ДекодованіДані) ЗнайтиМаскуВиключногоАбо(байт[] дані)
{
    ...
    for (ціл маска = 0; маска <= 255; маска++)
    {
        ...
        ціл рахунок = (КількістьСимволів(замаскованіДані, ' ') << 4) + КількістьСимволів(замаскованіДані, 'a') + КількістьСимволів(замаскованіДані, 'e');
...
ціл КількістьПробілів(байт[] дані) => КількістьСимволів(дані, ' ');

ціл КількістьСимволів(байт[] дані, симв символ)
{
    ціл результат = 0;
    for (var і = 0; і < дані.Length; і++)
    {
        if ((симв)дані[і] == символ)
        {
            результат++;
        }
    }
...
байт[] ВиключенеАбоІзПовторюванимКлючем(байт[] ціль, байт[] маска)
{
    байт[] результат = new байт[ціль.Length];
    for (var і = 0; і < ціль.Length; і++)
    {
        результат[і] = (байт)(ціль[і] ^ маска[і % маска.Length]);
    }
...
байт[] ПереконвертуватиРядок(рядок дані)
{
    Список<байт> результат = new();
    for (ціл i = 0; i < дані.Length; i += 1)
    {
        результат.Додати((байт)дані[i]);
    }
